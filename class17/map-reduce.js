...
const shiftLetters = function(string){
  var charCodeList = string.split("").map(x => x.charCodeAt()+1);
  var charList = charCodeList.map(x => String.fromCharCode(x))
  return charList.join("")
}


var shift = shiftLetters('world')
console.log(shift)
...
console.log(countLetters("daasbbbccc"))

/* Write a function that takes a string and a target, and returns true or false if the target is present in the string. Use .reduce to accomplish this.
...
const isPresent = function(string, target) {
  return string.split("").reduce((accumulator, currentValue) => {
    //console.log(accumulator, currentValue)
      accumulator = accumulator || target === currentValue
      return accumulator
  }, false)
}

console.log(isPresent("jsdkah", "d"))
...
const decode = function(string){
  var decodedString = string.split(" ").map(x => {
    //console.log(`x:${x}`)
    var myCharCode = x.split("").reduce((accumulator, currentValue) => {
      //console.log(accumulator)
      accumulator = accumulator + parseInt(currentValue)
      return accumulator
    },0)
    return String.fromCharCode(myCharCode)
  })
